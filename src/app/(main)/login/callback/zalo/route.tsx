import { getJwtTokenFromZaloCode } from '@/api-services/auth-service';
...
export async function GET(request: NextRequest, context: { params: any }) {
  const zaloToken = request.nextUrl.searchParams.get('code') ?? '';
  const redirectUrl = `${process.env.NEXT_PUBLIC_DOMAIN}/login/callback/zalo`;
  const userProfile = await getJwtTokenFromZaloCode(zaloToken, redirectUrl);
